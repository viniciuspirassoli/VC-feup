...
def rbl(img, kernel_size, sigma_color = 0.5, sigmal_space = 0.5, sigma_r = 0.5):
    
    img = cv2.cvtColor(img, cv2.COLOR_BGR2YCR_CB)
    output = np.zeros_like(img)

    border_size = int((kernel_size-1)/2)
    padded = cv2.copyMakeBorder(img, border_size, border_size, border_size, border_size, cv2.BORDER_REFLECT_101)
    for x in range(border_size, output.shape[0]+border_size-1):
        for y in range(border_size, output.shape[1]+border_size-1):
            roi = padded[x-border_size:x+border_size+1, y-border_size:y+border_size+1]
            kernel = m.create_bilateral_kernel(roi)
            output[x, y] = np.sum(roi*kernel)
...
def robust_bilateral_filter(img, d, sigma_color, sigma_space, sigma_r):
    ...
    range_kernel = np.broadcast_to(range_kernel[:,:,np.newaxis], (7, 7, 3))
    ...
    for i in range(d, img.shape[0]):
        for j in range(d, img.shape[1]):
            ...
            photometric_similarity = np.array(np.exp(-0.5 * ((region - img[i-d:i+d+1, j-d:j+d+1])**2) / sigma_color**2))
            # Compute geometric closeness
            geometric_closeness = np.sqrt((x**2 + y**2)) * spatial_kernel
            geometric_closeness = np.broadcast_to(geometric_closeness[:,:,np.newaxis], (7, 7, 3))
            # Compute weights
